    ...
    def from_mq_to_db
      ...
      EM.run do
        ...
        queue.bind("#{@queueprefix}_tcppacket").subscribe do |metadata, body|
          ...
          if tcpdatadim["tcp_ack"] == "true" 
            tcp_packet << "'Y',"
          else
            tcp_packet << "'N',"
          end
          if tcpdatadim["tcp_fin"] == "true"
            tcp_packet << "'Y',"
          else
            tcp_packet << "'N',"
          end
          if tcpdatadim["tcp_syn"] == "true" 
            tcp_packet << "'Y',"
          else
            tcp_packet << "'N',"
          end
          if tcpdatadim["tcp_rst"] == "true" 
            tcp_packet << "'Y',"
          else
            tcp_packet << "'N',"
          end
          if tcpdatadim["tcp_psh"] == "true" 
            tcp_packet << "'Y',"
          else
            tcp_packet << "'N',"
          end
          if tcpdatadim["tcp_urg"] == "true"
            tcp_packet << "'Y',"
          else
            tcp_packet << "'N',"
          end
